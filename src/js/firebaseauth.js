...
function voterlogin() {
  ...
  firebase.auth().signInWithEmailAndPassword(vcnic, vpin)
    .then((userCredential) => {
      // Signed in 
...
function registerVoter() {
  ...
  if (vrmail && vrcnic && vrphone && vrpin) {
    ...
    if (vrcnic.length == 13) {
      ...
      firebase.auth().createUserWithEmailAndPassword(vrmail, vrpin)
        .then((userCredential) => {
          // Signed in 
          var user = userCredential.user;
    ...
    } else { window.alert("Invalid Cnic") }
...
function statusActive() {
  ...
  document.getElementById("elecmsg").innerHTML = "Elections active";


  var db = firebase.firestore();
  db.collection("electionstatus").doc("GZfNxyDWkg0RTS9Z2Wya").update({
    value: 1
    //Pin: vrpin
  })
    .then((docRef) => {
...
function statusClosed() {
  ...
  document.getElementById("elecmsg").innerHTML = "Elections Closed";
...
setInterval(() => {
  ...
  document.getElementById('timecl').innerHTML = mydate.getHours() + ":" + mydate.getMinutes() + ":" + mydate.getSeconds() + "<br>" + mytime;
